...
def load_pics(file_name):
	pygame.transform.scale2x(pygame.image.load(os.path.join("imgs", file_name)))
...
class Bird:
	...
	def draw(self, win):
		...
		# 	self.img = self.IMGS[0]
		# 	self.img_count = 0
		img_num = math.floor((self.img_count%(3* self.ANIMATION_TIME))/5)
		self.img = self.IMGS[img_num] 
		if self.tilt <= -80:
			self.img = self.IMGS[1]
